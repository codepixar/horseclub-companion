...
function horseclub_import_files() {
	...
	$demoImg = '<img src="'.plugins_url( 'screen-image.png', __FILE__ ) .'" alt="'.esc_attr__( 'Demo Preview Imgae', 'horseclub-companion' ).'" />';
  ...
  return array(
    array(
      ...
      'import_customizer_file_url'   => plugins_url( 'horseclub-customizer.dat', __FILE__ ),
...
function horseclub_import_plugin_page_setup( $default_settings ) {
	...
	$default_settings['page_title']  = esc_html__( 'One Click Demo Import' , 'horseclub-companion' );
	$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'horseclub-companion' );
...
function horseclub_demo_import_custom_scripts(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'horseclub-demo-import' ){
		// style
		wp_enqueue_style( 'horseclub-demo-import', plugins_url( 'css/demo-import.css', __FILE__ ), array(), '1.0', false );
	}
