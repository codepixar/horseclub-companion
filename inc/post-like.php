...
function horseclub_sl_enqueue_scripts() {
	...
	wp_enqueue_script( 'horseclub-post-likes', plugins_url( '../js/post-likes.js', __FILE__ ), array( 'jquery' ), '0.5', false );

	wp_localize_script( 'horseclub-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'horseclub-core' )
	) );
}
...
function horseclub_process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'horseclub-core' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'horseclub-core' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'horseclub-core' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'horseclub-core' );
...
function horseclub_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'horseclub-core' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'horseclub-core' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
